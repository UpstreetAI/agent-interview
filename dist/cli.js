...
var generateFeaturePrompt = (featureSpecs2, userSpecifiedFeatures, allowAll) => {
  const prompt = allowAll ? dedent3`\
      The available features are:
    ` + "\n" + featureSpecs2.map(({ name, description }) => {
    return `# ${name}
${description}`;
  }).join("\n") + "\n\n" : dedent3`\
      The agent is given the following features:
    ` + "\n" + Array.from(userSpecifiedFeatures).map((feature) => {
    const spec = featureSpecs2.find((spec2) => spec2.name === feature);
